...
The next step here is to replace the ciphertext message memory with something that can be dynamically set during runtime and feed every core simultaneously. This also opens the door to something like the DE-1's Cortex A9 which could be used to control the FPGA and set new encrypted messages during system operation, thus allowing more advanced applications in the future such as RC4 encrypted packet processing.
